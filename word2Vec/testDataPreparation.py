...
class Document2Vec:
    def train_model(self, embedding_size):
        word2vec.word2vec('data/train.txt', 'data/word2vec.bin', size=embedding_size, verbose=True)
    ...
    def obtain_sentence_list (self):
        ...
        file = codecs.open('data/testTextWord.txt','r', 'utf-8')
    ...
    def predict_text2vec_obtain_text_source (self, embedding_size):
        ...
        np.savetxt('data/testText.txt', sentence_vecs)
        np.save('data/zeroLenListTest.npy', zero_len_list)
...
embedding_size = 100
doc2vec.train_model( embedding_size )
zero_len_list = doc2vec.predict_text2vec_obtain_text_source( embedding_size )
temp = np.load('data/zeroLenListTest.npy')
print temp
print (len(temp))
